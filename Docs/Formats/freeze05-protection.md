...
| Lv2 | Lv1 + 게임 정상 실행 | ❌ 미구현 | EUD VM을 살려둔 채 unFreeze만 무력화 필요 |
| Lv3 | 완전 복원 (원본과 동일) | ❌ 미착수 | Freeze 보호 흔적 완전 제거 |

**현재 상태**: Lv1까지 완료. 에디터에서 트리거를 읽고 편집할 수 있으나 게임은 실행 불가.
Lv2가 어려운 이유는 EUD VM 내에서 unFreeze 부분만 분리해야 하고,
오프셋 암호화 등 다른 보호 메커니즘은 그대로 유지해야 하기 때문.
...
1. encrypted trigger body key를 brute-force로 복구한다.
2. trigger body를 복호화한다.
...
런타임 덤프는 여전히 유용한 fallback이지만, 주 복구 전략은
`파일 단독 triggerKey 직접 탐색 + type-byte targeted partial decrypt 검증`이다.
...
| `ApplyRuntimeDump(...)` | CE 메모리 덤프 적용 (폴백) |
...
파이프라인을 구현했다. 이 경로는 파일 단독 `triggerKey` 직접 탐색으로
key를 찾지 못했을 때 사용하는 fallback이다.
...
| 맵 | 결과 |
|-----|------|
| `!! 저글링_키우기_ver25.scx` | trigger 72 복호화 성공, ScmDraft2에서 정상 확인 |
...
**권장**: 가능하면 StarCraft 1.16.1에서 덤프. EUD 트리거가 실제 트리거 메모리를 직접 수정하므로
CE 덤프가 확실하게 동작한다.
...
5. [ ] **type-byte targeted partial decrypt 설계**: wlist가 건드리는 dword 중 condition/action type byte만 빠르게 검사
6. [ ] **파일 단독 triggerKey 직접 탐색 구현**: allocation 없는 병렬 2^32 탐색 + 빠른 reject + 전체 trigger 검증
7. [ ] **전체 encrypted TRIG 복구**: 검증된 `triggerKey`로 모든 encrypted trigger 복호화
8. [ ] **런타임 덤프 fallback 유지**: 직접 key 탐색 실패 시 `--apply-dump` 경로 사용
